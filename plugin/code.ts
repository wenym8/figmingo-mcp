...
const handlers: Record<string, (params: any, vars: Map<string, string>, ctx?: RunCtx) => Promise<any>> = {
  ...
  async create_text(params, vars, ctx) {
    ...
    const metricCompensate = () => {
      ...
      if (!params.width || !text.characters) return;
      const charCount = Array.from(text.characters).length;
      if (charCount < 3) return;
      const overflow = text.width - params.width;
      if (overflow < 1) return; // smaller than extraction is harmless
      const fontSize = typeof text.fontSize === 'number' ? text.fontSize : 16;
      const existing =
        typeof text.letterSpacing === 'object' && text.letterSpacing.unit === 'PIXELS' ? text.letterSpacing.value : 0;
      ...
      if (Math.abs(adjust - existing) < 0.05) return;
    ...
    const reanchor = () => {
      ...
      if (params.anchorRight && params.width) {
        ...
        if (Math.abs(dx) >= 0.25) text.x = params.x - dx;
      }
      if (params.height) {
        const dy = (text.height - params.height) / 2;
        if (Math.abs(dy) >= 0.25) text.y = params.y - dy;
      }
    };
    reanchor();
    ...
    if (params.runs?.length) reanchor();
    return {
      nodeId: text.id,
      fontApplied: font,
